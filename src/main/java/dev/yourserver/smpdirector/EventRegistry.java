...
public class EventRegistry {
    ...
    public void reloadSettings() {
        ...
        this.evalEverySeconds = Math.max(1, cfg.getInt("global.evaluateEverySeconds", 7));
        this.minSecondsBetween = Math.max(0, cfg.getInt("global.minSecondsBetweenEvents", 90));
        this.minSecondsBetweenJitter = Math.max(0, cfg.getInt("global.minSecondsBetweenEventsJitter", 60));
    ...
    public boolean shouldEvaluateTick(int tickSeconds){ return (tickSeconds % Math.max(1, evalEverySeconds)) == 0; }
    ...
    public void markAny(Player p){ lastAny.put(p.getUniqueId(), System.currentTimeMillis()); }

    public double chanceFor(double tensionVal){
        String band = bandOf(tensionVal);
        switch (band) { case "calm": return chanceCalm; case "high": return chanceHigh; default: return chanceNormal; }
    }

    public DirectorEvent pickFor(Player p) {
        double val = plugin.getTensionManager().get(p.getUniqueId());
        String band = bandOf(val);
        Map<DirectorEvent, Integer> weights = new HashMap<>();
        int total = 0;
        for (DirectorEvent ev : events) {
            int w = getWeight(ev.id(), band);
            if (w > 0 && ev.canRunFor(p) && !onCooldown(ev, p)) { weights.put(ev, w); total += w; }
        }
        if (total <= 0) return null;
        int r = random.nextInt(total), acc = 0;
        for (Map.Entry<DirectorEvent, Integer> e : weights.entrySet()) { acc += e.getValue(); if (r < acc) return e.getKey(); }
        return null;
    }
    ...
    public void markRun(DirectorEvent ev, Player p) {
        ...
        markAny(p);
        int jitterMs = (minSecondsBetweenJitter > 0) ? new Random().nextInt(minSecondsBetweenJitter * 1000 + 1) : 0;
        nextAllowedAt.put(p.getUniqueId(), now + (minSecondsBetween * 1000L) + jitterMs);
    }
