...
public class ReliefDropEvent implements DirectorEvent {
        ...
        for (String spec : pool) {
            ...
            try {
                ...
                if (parts.length >= 2) {
                    ...
                    if (range.length == 2) { min = Integer.parseInt(range[0]); max = Integer.parseInt(range[1]); }
                    else { max = Integer.parseInt(parts[1]); }
                }
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                ...
                if (state2 instanceof Barrel) {
                    ...
                    if (inv2.isEmpty()) {
                        ...
                        fw.spawnParticle(Particle.CLOUD, fdrop.clone().add(0.5, 0.5, 0.5), 10, 0.3, 0.2, 0.3, 0.01);
                    }
                }
            }, removeAfter * 20L);
        }
    ...
    @Override public int cooldownSeconds() { return plugin.getConfig().getInt("events.relief_drop.cooldownSeconds", 600); }
}
