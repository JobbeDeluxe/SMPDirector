...
public class TensionManager {
    ...
    public TensionManager(SMPDirectorPlugin plugin) { this.plugin = plugin; }
    ...
    public void decayAll() {
        ...
        for (UUID id : new ArrayList<>(map.keySet())) set(id, get(id) - decay);
    }

    public void tickSensedInputs(Player p) {
        if (p.getFoodLevel() <= 7) add(p.getUniqueId(), plugin.getConfig().getDouble("tension.lowFood", 3));
        int light = p.getLocation().getBlock().getLightLevel();
        if (light <= 7) add(p.getUniqueId(), plugin.getConfig().getDouble("tension.darkness", 2));
        int radius = plugin.getConfig().getInt("tension.nearMobRadius", 12);
        int count = 0;
        for (org.bukkit.entity.Entity e : p.getNearbyEntities(radius, radius, radius)) {
            if (e instanceof LivingEntity && !(e instanceof Player)) count++;
        }
        double factor = plugin.getConfig().getDouble("tension.nearMobFactor", 0.5);
        add(p.getUniqueId(), count * factor);
    }

    public void showOrUpdateBossbar(Player p) {
        if (!plugin.getConfig().getBoolean("global.bossbar.enabled", true)) { hideBossbar(p.getUniqueId()); return; }
        ...
        String title = plugin.getConfig().getString("global.bossbar.title", "&bTension: &f%val%").replace("&","§").replace("%val%", String.format("%.0f", val));
        bar.setTitle(title);
        if (!bar.getPlayers().contains(p)) bar.addPlayer(p);
    }

    public void hideBossbar(UUID id){ BossBar b = bars.remove(id); if (b != null) b.removeAll(); }
}
